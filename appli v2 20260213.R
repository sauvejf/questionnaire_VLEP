...
parse_hazard_codes <- function(txt) {
  ...
  # Supprime les espaces entre lettre et chiffres (ex: H 361 d -> H361d)
  txt_up <- gsub("\\s+", "", txt_up)
  
  # Capture H### ou EUH### même si suffixe derrière (fd, d, f, etc.)
  matches <- gregexpr("\\bEUH\\d{3}[A-Z]*\\b|\\bH\\d{3}[A-Z]*\\b", txt_up)
  tokens_full <- unlist(regmatches(txt_up, matches))
  
  if (length(tokens_full) == 0) return(character(0))
  
  # Normalisation : garder uniquement le préfixe + 3 chiffres
...
.mono { font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace; }
  ...
  # Main content (full width) with right padding reserved for TOC
...
server <- function(input, output, session) {
  ...
  lapply(seq_along(agents), function(i) {
    local({
      ...
      output[[hazard_summary_output]] <- renderUI({
        ...
        if (is.null(out) || length(out$codes) == 0) {
          return(tags$div(
            class = "label-box",
        ...
        tags$div(
          ...
          tags$p(tags$strong("Mentions prises en compte : "),
                 tags$span(class = "mono", paste(out$codes, collapse = ", "))),
  ...
  is_done <- function(v, out_applied = NULL) {
    ...
    return(!is.na(bd) && is.finite(as.numeric(bd)) && length(out$codes %||% character(0)) > 0)
  }
  ...
  observeEvent(input$load_session, {
    ...
    lapply(seq_along(agents), function(i) {
      ...
      haz[[paste0("out_", i)]] <- compute_from_codes(v$hcodes %||% character(0))
