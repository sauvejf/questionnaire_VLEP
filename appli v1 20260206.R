...
scale_labels <- c(
  "1" = " > 5 mg/m³ (aérosols), ≥40 ppm (gaz/vapeurs)",
  "2" = "]0,5-5] mg/m³ (aérosols), [2-40[ ppm (gaz/vapeurs)",
  "3" =  "]0,05-0,5] mg/m³ (aérosols), [0,1-2[ ppm (gaz/vapeurs)",
  "4" =  "]0,005-0,005] mg/m³ (aérosols), [0,005-,01[ ppm (gaz/vapeurs)",
  "5" = "≤0,005 mg/m³ (aérosols), <0,005 ppm (gaz/vapeurs)" 
)
      ...
      lapply(seq_along(agents), function(i) {
        ...
        tags$div(
          ...
          conditionalPanel(
            ...
            radioButtons(
              inputId = scale_input,
              label = "Sélectionnez une bande de danger (1–5)",
              choices = 1:5,
    ...
    lapply(seq_along(agents), function(i) {
      local({
        ...
        output[[scale_label_output]] <- renderUI({
          ...
          tags$div(
            class = "label-box",
            tags$strong("Bande de concentration : "),
